...
SELECT COUNT([Action]) AS TOTAL_ACTION
FROM Actions.Actions

SELECT Adv_Type, COUNT(*) Orders
FROM Actions.Actions
GROUP BY Adv_Type
...
SELECT *
 FROM(
	SELECT Adv_Type, COUNT([Action]) AS ORDERS, (SELECT COUNT(*)
					FROM Actions.Actions) AS TOTAL_ACTION
	FROM Actions.Actions
	GROUP BY Adv_Type) AS A;

SELECT A.Adv_Type, A.ORDERS/TOTAL_ACTION *1.0 AS CONVERSION_RATE
 FROM(
	SELECT Adv_Type, COUNT([Action]) AS ORDERS, CAST((SELECT COUNT(*)
					FROM Actions.Actions) AS INT) AS TOTAL_ACTION
	FROM Actions.Actions
	GROUP BY Adv_Type) AS A;


SELECT Adv_Type, CAST(COUNT([Action]) AS numeric(4, 2))/CAST((SELECT COUNT(*)
					FROM Actions.Actions) AS numeric(4, 2))*1.00 AS CONVERSION_RATE
	FROM Actions.Actions
	GROUP BY Adv_Type
...
SELECT A.Adv_Type, A.COUNT_ORDER, B.ORDERS_TOTAL,
	CAST(((A.COUNT_ORDER*1.0)/(B.ORDERS_TOTAL*1.0)) AS FLOAT) AS CONVERSION_RATE
FROM (
